...
public class DatHangNoLoginActivity extends AppCompatActivity implements DatHangAdapter.DatHangCallBack{
    private EditText editTextName,editTextAddress, editTextPhone, editTextEmail;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_dat_hang_no_login);
        ...
        editTextName = findViewById(R.id.payment_tennguoidung);
        editTextAddress = findViewById(R.id.payment_diachi);
        editTextPhone = findViewById(R.id.payment_sdt);
        editTextEmail = findViewById(R.id.payment_email);
        ...
        APIClient.getInstance().getMyApi().getPaymentMethods().enqueue(new Callback<List<PaymentMethod>>() {
            ...
            public void onResponse(Call<List<PaymentMethod>> call, Response<List<PaymentMethod>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    Toast.makeText(DatHangNoLoginActivity.this, "Error: Unable to get payment methods", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onFailure(Call<List<PaymentMethod>> call, Throwable t) {
                Toast.makeText(DatHangNoLoginActivity.this, "Failure: " + t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void populatePaymentMethods(List<PaymentMethod> paymentMethods) {
        for (PaymentMethod method : paymentMethods) {
            RadioButton radioButton = new RadioButton(DatHangNoLoginActivity.this);
    ...
    private void submitOrder() {
        ...
        int idMethod = getIdMethodFromButton(paymentMethodButton); // Phương thức này lấy id từ tag của RadioButton

        // Gửi thông tin đơn hàng lên API
        APIClient.getInstance().getMyApi().addOrderNoLogin(email, phone, address, name, idMethod).enqueue(new Callback<ApiResponse>() {
            @Override
            public void onResponse(Call<ApiResponse> call, Response<ApiResponse> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ApiResponse apiResponse = response.body();
                    Toast.makeText(DatHangNoLoginActivity.this, apiResponse.getMessage(), Toast.LENGTH_LONG).show();
                    // Kiểm tra status và xử lý kết quả phản hồi tương ứng
                    ...
                    Toast.makeText(DatHangNoLoginActivity.this, "Lỗi khi đặt hàng", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onFailure(Call<ApiResponse> call, Throwable t) {
                // Xử lý khi không thể gửi yêu cầu lên API
                Toast.makeText(DatHangNoLoginActivity.this, "Lỗi kết nối: " + t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void showSuccessDialog() {
        new AlertDialog.Builder(this)
                ...
                .setMessage("Cảm ơn bạn đã đặt hàng !!!(Hãy đăng nhập để theo dõi đơn hàng nhé)")
                .setPositiveButton("OK", new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        // Gửi Intent trở lại MainActivity với thông tin bổ sung
                        Intent intent = new Intent(DatHangNoLoginActivity.this, MainActivity.class);
